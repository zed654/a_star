...
public:
    ...
    HybridAstar(double init_x_, double init_y_, double target_x_, double target_y_)
        : threshold_distance_for_stop(100),
          init_pos_index(0),
          oc_size_row(5000),
          oc_size_col(5000),
          negative_val_offset(3000),
          length_arc(10.1), //length_arc(5.3)
          ...
          init_offset_y(init_y_)
    {
        ...
        // FIXME:
        init_node_tmp.pos = {init_offset_x, init_offset_y, init_offset_x + 800, init_offset_y + 800}; // Init Pos {xn, yn, xc, yc}
    ...
    double GetPotentialCost(Eigen::VectorXd coeffs_, double x_, double y_)
    {
        ...
        double gradiant_constant = 20;
        double gradiant_intensity = 3;
        // double WPsEval_tmp = std::pow(PolyEval(coeffs_, x_-offset_tmp) - y_, 2);
        double WPsEval_tmp = std::pow(PolyEval(coeffs_, x_) - y_, 2);

        // bool inverse_flag_tmp = false;
        // double WPsEval_tmp = 0;
        // if(inverse_flag_tmp == false)
        //     WPsEval_tmp = std::pow(PolyEval(coeffs_, x_) - y_, 2);
        // else if(inverse_flag_tmp == true)
        //     WPsEval_tmp = std::pow(PolyEval(coeffs_, y_) - x_, 2);

        double GradientEval_tmp = -std::sqrt((x_ - offset_tmp) * (x_ - offset_tmp) + (y_ - offset_tmp) * (y_ - offset_tmp));
        double Offset_tmp = 5;
        // TODO: gradiant_constant, gradiant_intensity는 HyperParameter임.
        // double tmp = WPsEval_tmp / gradiant_constant + GradientEval_tmp * gradiant_intensity + Offset_tmp;
        double tmp = WPsEval_tmp + GradientEval_tmp * gradiant_intensity + Offset_tmp;
